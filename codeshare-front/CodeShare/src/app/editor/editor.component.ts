...
export class EditorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  checkTheme() {
    (document.querySelector('.comment') as HTMLDivElement)?.classList.remove(
      'comment-dark'
    );
    (document.querySelector('.comment') as HTMLDivElement)?.classList.remove(
      'comment-light'
    ...
    (
      document.querySelector('.option-panel') as HTMLDivElement
    ).classList.remove('option-panel-light');
    (
      document.querySelector('.option-panel') as HTMLDivElement
    ).classList.remove('option-panel-dark');
    if (AppComponent.ThemeType === 'light') {
      (document.querySelector('.comment') as HTMLDivElement)?.classList.add(
        'comment-light'
      );
      (document.querySelector('.option-panel') as HTMLDivElement).classList.add(
        'option-panel-light'
      );
    } else {
      (document.querySelector('.comment') as HTMLDivElement)?.classList.add(
    ...
    this.editorTheme =
      AppComponent.ThemeType === 'light' ? 'chrome' : 'tomorrow_night_eighties';
  }
    ...
    if (this.shortCode && !this.wsBody$) {
      setTimeout(() => {
        document
          ...
          .scrollTo(0, document.querySelector('.comment').scrollHeight);
      }, 500);
    }
  ...
  getShareUrl(): string {
    if (this.shortCode)
      return location.protocol + '//' + location.host + '/' + this.shortCode;
    return '';
  }
  ...
  downloadProjectImage() {
    domtoimage.toPng(document.querySelector('ace')).then((dataUrl) => {
      ...
      link.download = `codeshare-project${
  ...
  downloadProjectFile() {
    ...
    link.download = `codeshare-project${
      this.shortCode ? `-${this.shortCode}` : ''
    }.${FilenameMapping.getExtension(this.editorMode)}`;
    link.href =
  ...
  sendMsg() {
    ...
    AppComponent.SendSocketMessage(
      '/ws/contentBody/' + this.shortCode,
      JSON.stringify(wsProject)
    );
  }
  ...
  showReadonly() {
    if (this.shortCode && this.project)
      return AppComponent.CompareUUID(this.project.UUID);
    return !this.readonly;
  }

  async dynamicImportMode() {
    await import(`brace/mode/${this.editorMode}`);
    this.internalEditorMode = this.editorMode;
  }
}
