...
export class AppComponent implements OnInit {
  static API_POTO = window.location.host.includes('localhost')
    ? 'http'
    : 'https';
  static WS_POTO = window.location.host.includes('localhost') ? 'ws' : 'wss';
  static API_HOST = window.location.host.includes('localhost')
    ? 'localhost:8080'
    : 'codeshare.stw.tw:8443';
  static readonly API_PREFIX =
    AppComponent.API_POTO + '://' + AppComponent.API_HOST + '/';
  static readonly WS_PREFIX =
    AppComponent.WS_POTO + '://' + AppComponent.API_HOST + '/';
  ...
  static ChangeTheme(theme: Theme) {
    (
      document.getElementById('themeAsset') as HTMLLinkElement
    ).href = `assets/${theme}`;
    AppComponent.ThemeType =
      theme === Theme.DeeppurpleAmber || theme === Theme.IndigoPink
        ? 'light'
        : 'dark';
  ...
  static GetHashUUID(): string {
    return sha256(localStorage.getItem(AppComponent.LS_KEY_UUID));
  }
  ...
  ngOnInit(): void {
    ...
    // localstorage
    if (!localStorage.getItem(AppComponent.LS_KEY_UUID))
      AppComponent.GenerateUUID();
    if (!localStorage.getItem(AppComponent.LS_KEY_USER))
      localStorage.setItem(AppComponent.LS_KEY_USER, 'Anonymous');
    if (!localStorage.getItem(AppComponent.LS_KEY_THEME))
      localStorage.setItem(AppComponent.LS_KEY_THEME, Theme.PurpleGreen);
    AppComponent.ChangeTheme(
      localStorage.getItem(AppComponent.LS_KEY_THEME) as Theme
    );
  }
}
